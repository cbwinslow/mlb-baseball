...
- `tests/integration/` — anything that touches Postgres. Automatically provisions an isolated, disposable `mlb_test_<hex>` database per run via `pytest-postgresql` (never touching production `mlb`) — see `README.md` "Testing". Mock the network (fixture CSV/JSON content), not the database — real Postgres is cheap to run against locally and mocking it hides real bugs (e.g. transaction/lock behavior, COPY column mismatches).
