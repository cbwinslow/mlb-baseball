...
def mlb_schedule(
    ...
    team_id: str | None = typer.Option(None, "--team", "-t", help="Team ID"),
...
def retrosheet_events(
    season: int = typer.Option(..., "--season", "-s", help="Season year"),
    teams: list[str] | None = typer.Option(None, "--teams", "-t", help="Team codes"),
...
def statcast_pitcher(
    ...
    season: int | None = typer.Option(None, "--season", "-s", help="Season"),
    output_dir: Path = typer.Option(Path("data/raw/statcast"), "--output", "-o"),
) -> None:
...
@app.command()
def espn_scoreboard(
    date_str: str | None = typer.Option(None, "--date", "-d", help="Date (YYYYMMDD)"),
