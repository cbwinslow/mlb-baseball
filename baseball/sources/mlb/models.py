...
class MLBScheduleRequest(BaseModel):
    ...
    start_season: Optional[int] = Field(None, ge=1900, description='Start season for range')
    end_season: Optional[int] = Field(None, ge=1900, description='End season for range')
    team_id: Optional[str] = Field(None, description='Team ID (e.g., "NYY")')
    start_date: Optional[date] = Field(None, description='Start date (ISO format)')
    end_date: Optional[date] = Field(None, description='End date (ISO format)')

    @field_validator('start_season', 'end_season')
    @classmethod
    def validate_season_range(cls, v: Optional[int]) -> Optional[int]:
...
class MLBLiveRequest(BaseModel):
    ...
    max_polls: Optional[int] = Field(None, description='Max number of polls')
    timeout_seconds: Optional[int] = Field(None, description='Polling timeout in seconds')
