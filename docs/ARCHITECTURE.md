...
- **`gold`** — schema created empty (`migrations/0004_core_gold_schemas.sql`, ADR-013), reserved for ML-feature/serving-shaped tables until Phase 2/3 actually needed them. No longer empty. Two distinct families of table live here now, built by two different, independent processes (neither one wired into the other):
  - **Phase 2 predictive feature store**: `gold.game_feature` (migrations 0012-0022, ADR-032 onward) — one row per game, only pre-game-knowable columns, built by `mlb_baseball/model/*.py`'s `compute()` functions (`mlb predict`). `gold.prediction` (migration 0013) — one row per (game, model, generated_at), history-preserving. Both carry real FKs to `core.player`/`core.team`/`core.game`/`core.venue` (`gold.prediction`'s was later dropped in favor of `mlb_game_pk` text, since a still-upcoming game's prediction has no `core.game` row to reference yet).
  - **Reporting surface** (ADR-054, `mlb_baseball/report.py`, `mlb report` — not yet wired into the CLI, see that ADR): `gold.player_season`, `gold.team_season`, `gold.division_standing` (migration `0030_gold_reporting.sql`) — one flat table per entity/grain, built from already-conformed `core` plus already-computed `model/*.py` season aggregates, so a future Phase 3 API/website can query one table per page instead of assembling from a dozen raw/core tables at request time. Deliberately carry **no** FK to `core.player`/`core.team` — see ADR-054 for why (a real constraint from `conform.py`'s own consolidated `TRUNCATE` statement, not a style choice).
