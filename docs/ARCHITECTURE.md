...
- **Once a day — ingestion, then conform, then predict.** `scripts/mlb_daily_update.sh` runs three ordered but **independently tracked** steps (same `flock` + logging shape, logs to `logs/mlb_daily_update.log`): `mlb update --skip mlb_api` → `mlb conform` → `mlb predict` (ADR-032). Each step is timestamped with its own exit code; a partial failure in one still attempts the next (gold is worth rebuilding from whatever raw data landed), and the script's overall exit code is non-zero if any step failed. `--skip mlb_api` because the 5-minute cron above already owns MLB API freshness and almost always holds that connector's ingestion lock at 06:00 — the daily run's own `mlb_api` step used to fail on "another ingestion run is already active" every single day. Every connector's `update()` is deliberately scoped to be cheap — current season only, or a small full-catalog re-check, never a full historical re-fetch (see each connector's `bootstrap()`/`update()` split, e.g. `statcast.py`, `bref.py`, `statcast_leaderboard.py`) — so running all of them daily is safe. This is what keeps Statcast leaderboards, Baseball-Reference season stats, Retrosheet's current-decade archive, and `gold.game_feature`/`gold.prediction` from going stale as a season progresses.
- **Once a day — full backup.** `scripts/mlb_backup.sh` runs `mlb backup --keep 7` (same `flock` + logging shape, logs to `logs/mlb_backup.log`): a full `pg_dump`, then deletes full backups beyond the newest 7. The backup itself records a `meta.ingestion_run` row (`backup.py`), so `mlb doctor` can flag a stale backup the same way it flags a stale connector run — added after a real gap was found: the only backup on disk was a single manual dump going stale with nothing rotating or re-running it.

None of these scripts are installed to crontab automatically — see the "Bootstrap procedure" section below and CLAUDE.md's action-confirmation rules; a person (or agent, with explicit confirmation) adds the crontab line.
