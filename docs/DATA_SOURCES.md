...
| **MLB Stats API** (`statsapi.mlb.com`) | Full-history schedule (1901–present, incl. live game status), standings (1969–present, the divisional era), and append-only live-game snapshots (score, inning, balls/strikes/outs, current batter/pitcher) for any game currently in progress | Free, public, unauthenticated | `statsapi` Python package (`MLB-StatsAPI` on PyPI, `toddrob99/MLB-StatsAPI` — 830+ stars, actively maintained, GPL-3.0) | Full history landed even where Retrosheet already covers the same seasons — a second, independently-sourced copy is a cross-validation asset, and storage is cheap (see ADR-015, which supersedes ADR-014's original current-season-only scoping). Live-game capture is what actually supports real-time odds: everything else in this pipeline is completed-game-only. Getting continuously-fresh live data requires `mlb ingest mlb_api --mode update` to actually run repeatedly — the scheduling mechanism for that is still an open decision, not something this connector invents on its own. Boxscores/rosters deferred — see ADR-015 and `docs/ROADMAP.md`. |
