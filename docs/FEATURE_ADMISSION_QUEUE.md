...
| --- | --- | --- | --- | --- | --- |
| OFF-01 team prior OBP | game-team; `(H+BB+HBP)/(AB+BB+HBP+SF)` | Retrosheet/API events before game cutoff | NULL below denominator/min-sample; measure historical eras | no current-game PA; hand fixture and future-event exclusion | now / low — core formula implemented, `team_rate.py` (ADR-061); min-sample floor outstanding, only the `> 0` denominator guard landed; historical-era coverage measured 2026-08-13, no gap found — see [`docs/RAW_CORE_GOLD_FIELD_CENSUS.md`'s 2026-08-13 entry](RAW_CORE_GOLD_FIELD_CENSUS.md#2026-08-13--off-01-era-coverage-for-bat_event_flevent_cdab_flsf_fl) (github.com/cbwinslow/mlb-baseball/issues/8) |
| OFF-02 team prior SLG/ISO | game-team; total bases/AB; `ISO=SLG-AVG` | prior completed events | NULL below AB threshold | denominator guard and doubleheader ordering | now / low — core formula implemented, `team_rate.py` (ADR-061); AB threshold beyond `> 0` and a doubleheader-ordering test outstanding (github.com/cbwinslow/mlb-baseball/issues/8) |
| OFF-03 team prior BB%/K% | game-team; BB/PA and K/PA | prior completed events | retain PA and missing flag | event-code parity with hand-calculated fixture | now / low — core formula implemented, `team_rate.py` (ADR-061); retained PA/denominator column and missing flag outstanding (github.com/cbwinslow/mlb-baseball/issues/8) |
...
| OFF-08 run environment | game-team; prior runs/game and run differential | prior completed core games | first-game NULL retained | hand window + suspended/doubleheader test | now / low — core formula implemented, `team_rate.py` (ADR-061); suspended/doubleheader test outstanding (github.com/cbwinslow/mlb-baseball/issues/8) |
| DEF-01 run prevention | game-team; prior RA/game and opponent-adjusted variant | prior completed games | first-game NULL retained | distinguish pitching from defense in documentation | now / low — core formula implemented, `team_rate.py` (ADR-061), sharing OFF-08's run-environment computation and its outstanding suspended/doubleheader test; the pitching-vs-defense documentation distinction is also not yet written (github.com/cbwinslow/mlb-baseball/issues/8) |
