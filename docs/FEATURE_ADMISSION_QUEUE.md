...
| PIT-03 starter rest/workload | game-player; days since start, prior 7-day outs | starter as-of + previous completed appearances | NULL unknown starter / debut start | hand-computed fixture, zero-leakage, doubleheader ordering, debut NULL policy | now / medium — implemented, `starter_workload.py` (ADR-068, migration `0056`). Retrosheet-historical path only (`compute()`), parameterized single 7-day window (`home_starter_outs_7d`/`away_starter_outs_7d`) and rest days (`home_starter_rest_days`/`away_starter_rest_days`) using day-collapse RANGE-frame pattern at pitcher grain (ADR-042). Verified with hand-computed integration tests in `tests/integration/test_model_starter_workload.py` (a synthetic multi-start/relief/doubleheader fixture with worked-out arithmetic, not real-pitcher data — `starter.py`'s own reconciliation is the module that verifies against a real pitcher's real season, Jacob deGrom's 2018, a different check this row doesn't reuse). Live/probable paths deliberately deferred to follow-up package. Fully closed. |
