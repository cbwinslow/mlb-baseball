...
| DEF-01 run prevention | game-team; prior RA/game and opponent-adjusted variant | prior completed games | first-game NULL retained | distinguish pitching from defense in documentation | now / low — implemented, `team_rate.py` (ADR-061), sharing OFF-08's `compute_run_environment()` and its now-closed suspended/doubleheader test (`ee92003`, see OFF-08 row). Not part of issue #8's scope and still genuinely outstanding: the pitching-vs-defense documentation distinction this row's own "required test" column calls for has not been written — tracked as a real, separate gap, not claimed done here |
