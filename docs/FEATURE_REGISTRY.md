...
| `trend_v1` | Per side; `home_win_pct_trend`/`away_win_pct_trend` on `gold.game_feature` | Pure algebra (`win_pct_10 - win_pct`) over two already-approved base-family columns -- no new raw dependency, no join. Positive means playing better recently than season rate; negative means worse (ADR-083, admission queue INT-02). | NULL whenever either `win_pct` or `win_pct_10` is itself NULL (e.g. a team's first game of the season). | `tests/integration/test_model_trend.py` (hand-calculated fixture for both directions, NULL-propagation test, idempotency, health-check parity violation and clean-pass tests); `mlb_baseball/model/trend.py`, ADR-083. |
