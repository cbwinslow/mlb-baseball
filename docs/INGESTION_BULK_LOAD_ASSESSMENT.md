...
environment intermittently lost DNS resolution; after item-level failure
tracking was added, an eight-worker production recovery continued to make
durable progress with no failed items. The test-database write comparison was
0.017 seconds for individual commits and 0.013 seconds for one COPY on that
small payload. The unattended production backfill therefore defaults to eight
