...
it does not enumerate every file under `docs/superpowers/` individually
(see that section below for why).
...
The **research database is the current product focus**. The prediction ladder
and Astro site remain paused until the research database is a clean, useful
outside-user product.

Agents: read `NORTH_STAR.md` first, then this page, then `PRODUCT_DIRECTION.md`,
then the current research-database spec and September consolidation plan. Do not
add a metric Engine, GBM column, source, framework, or new product surface merely
because the repository can support it.

- [`NORTH_STAR.md`](NORTH_STAR.md) — the long-term vision, phased product idea,
  budget rule ($0/month), and differentiation goals.
- [`PRODUCT_DIRECTION.md`](PRODUCT_DIRECTION.md) — living product/architecture
  direction. Read this before reopening prediction/model/site scope.
- [`superpowers/specs/2026-09-01-research-database-v1-design.md`](superpowers/specs/2026-09-01-research-database-v1-design.md)
  — the current owner decision to finish the research database as one coherent
...
- [`research/feature_admission_sources.md`](research/feature_admission_sources.md)
  — **draft research input, not an approved contract.** Distinguishes a
  statistic's definition from proof this project has a lawful, complete,
  time-stamped input for it — read before proposing a new feature source.

The modeling surface is retained but paused as a product track while the research
DB is consolidated. Historical model docs remain valid evidence/architecture
records; they are not authorization to pull model work ahead of the current
research plan.
...
- [`DECISIONS.md`](DECISIONS.md) — **the ADR log.** Every non-trivial
  decision, newest first, and long — search it, don't read it front to
...
- [`AGENT_CONTEXT_ARCHITECTURE.md`](AGENT_CONTEXT_ARCHITECTURE.md) — proposed
  canonical `AGENTS.md` hierarchy, tool adapters, context-budget rules, and
  cross-project rollout model. The implementation pilot is
  [`superpowers/plans/2026-09-01-dox-agent-context-rollout.md`](superpowers/plans/2026-09-01-dox-agent-context-rollout.md).
...
`docs/superpowers/plans/` and `docs/superpowers/specs/` are per-session,
timestamped planning/design artifacts generated by an AI-assisted workflow.
They are normally not indexed file-by-file here. High-level current plans may be
linked in **Start here** when they materially define the active program; that does
not turn every historical session artifact into a living document.

If you're looking for the reasoning behind a specific past change, check the
dated filename against `plans/PROGRESS.md`'s dated entries first.

## Outside `docs/`

- [`../CLAUDE.md`](../CLAUDE.md) / [`../AGENTS.md`](../AGENTS.md) — operating
  rules for anyone (human or AI) working in this repo. Read before making
  changes, not after. These are candidates for the staged hierarchical context
  pilot described above; until that work merges, their current contents remain
  the operative repository instructions.
- [`../plans/PROGRESS.md`](../plans/PROGRESS.md) — the evidence log of what
  ...
  don't replace.
- [`../plans/*.md`](../plans/) — the numbered execution-plan sequence. Current
  product focus and newer dated plans take precedence when an older plan status
  section has not yet been reconciled.
