...
Not indexed file-by-file above, on purpose: `docs/superpowers/plans/` and
`docs/superpowers/specs/` are per-session, timestamped planning/design
artifacts generated by an AI-assisted workflow (one file per goal-seek
session or brainstormed spec), not curated reference docs — there's no
fixed set to list, new ones appear as work happens. If you're looking for
the reasoning behind a specific past change, check the dated filename
against `plans/PROGRESS.md`'s own dated entries first.
