...
5. ✅ **Team offensive true talent → team wOBA** — built (`mlb_baseball/model/offense.py`, ADR-036), better than originally planned: a genuine within-season rolling number from `raw.retrosheet_event`, not a season-lagged Statcast aggregate. FanGraphs' own published formula, recreated (not scraped — confirmed they don't support scraping/API access at all). Verified: real 2023 league-average wOBA computed at .317, matching the known real value; real 2024 team values all land in .295-.333.
6. **wRC+** — park- and league-adjusted extension of team wOBA, deliberately deferred (needs FanGraphs' "wOBA Scale" constant, same year-varying sourcing problem as the wOBA weights and the FIP constant — deserves its own verification pass).
7. **Bullpen quality/fatigue** — still open, real value per research, a distinct body of `core.play`/`raw.retrosheet_event` engineering work from starter quality.
8. **Defensive value via Statcast OAA** — `raw.statcast_oaa.fielding_runs_prevented` (2016-2026, already ingested) is a real, free, modern substitute for FanGraphs' UZR/DRS, which depend on proprietary data with **no free path to replicate — a permanent wall under the $0-budget rule, not a "not yet."** OAA needs the same prior-season-lag treatment as WAR (it's a season aggregate), not the within-season-rolling treatment starter quality/wOBA use.
9. **2026 (current-season) starter quality + wOBA from raw.mlb_playbyplay** — closes both starter.py's and offense.py's shared biggest practical gap (no signal for the live season `mlb predict` actually serves), separate parsing work against a different schema than Retrosheet's.
