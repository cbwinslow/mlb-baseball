...
- **Reality-check calibration**: several public repos claim 67-93%+ accuracy. These are almost certainly leakage artifacts (same-game box-score stats used as inputs), not something to benchmark against. **If our own model reports out-of-sample accuracy above ~58% (or a suspiciously low log loss), that's a trigger to run a leakage review — chronological folds, feature cutoffs, the failure modes below — before the number is trusted or promoted, not a result to celebrate. 70%+ is that alarm at full volume.** A review-triggering number is not automatically proof of leakage, and it does not automatically block the work (ADR-274) — a human looks at the evidence and records the call.
...
**Honest limitation of the `feat.*` feature store (feature-store-v1, ADR-287).** The point-in-time clock is an assumption, not a measurement. Retrosheet records neither a first-pitch time nor an ingest time. `event_ts` is `game_date + game_number × 3h` — a fictional absolute time that gets same-day (doubleheader) *ordering* right; every rolling window excludes the entering game's whole calendar day (ordered and bounded by `date_trunc('day', event_ts)`, not the raw fictional clock — an earlier version bounded by raw `event_ts` and a flat 6h lag, which let the window's own N-days-ago start wobble with the intraday offset; fixed 2026-09-23 after it broke `mlb verify`'s doubleheader check on the first full-scale production run). So real same-day timing (a rain delay, a split doubleheader) is not modelled — every leg of a multi-game day sees identical prior history — and a *full rebuild* cannot honour "a record that entered Retrosheet after 2015 is invisible to a 2015 decision" (Retrosheet backfills and corrects history and we do not know when each record landed). The `feat.*` layer is leak-free against its own clock model; that model is conservative but coarse. `mlb verify`'s two checks test the mechanism.
