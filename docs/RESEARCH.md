...
10. **2026 (current-season) starter quality + wOBA from raw.mlb_playbyplay** — closes both starter.py's and offense.py's shared biggest practical gap (no signal for the live season `mlb predict` actually serves), separate parsing work against a different schema than Retrosheet's.
