...
6. 🟡 **MLB Stats API connector** (`mlb_api`, ADR-014) — current season's schedule (incl. live status) and standings done, via the `statsapi` package. Verified against production: 2,946 games, 30 teams' standings for 2026, idempotent. Not done: boxscores, rosters, full live game state — deferred to their own connector(s) once there's a real need, same pattern as `retrosheet_box` being split from `retrosheet_event`.
