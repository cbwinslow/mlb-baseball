...
|---|---|---|---|
| `core.player` | One resolved person (`id`); provider IDs are alternate anchors. `retro_id` is UNIQUE where present but nullable — NULL for a current-season player admitted on their MLBAM id before Retrosheet has processed that season | Biography/current identity, not a player-season fact | Multi-pass source reconciliation in `conform.py` from `raw.register_people`: admitted on a Retrosheet id, or on an MLBAM id when the person appears in MLB's own game record (`raw.mlb_boxscore_*` / `raw.mlb_playbyplay`). Never admits an MLBAM-only register row with no MLB game appearance; never treats display name as an identity key. Truncate-and-rebuild every `mlb conform`, so a NULL `retro_id` backfills once Retrosheet catches up. `mlb doctor` enforces that every regular-season game participant resolves (migration 0103) |
...
The grain-complete statistic backbone rows below (`gold.batting_game` …
`gold.*_career`) are governed by
...
| `gold.player_season` | One provider final-season line per `(player_id, season, is_pitcher)` | Final-season aggregate; never a pregame feature | Explicit `mlb report` rebuild from Baseball-Reference season rows plus conformed WAR. **Regular season only** (incl. Game 163 tiebreaker) — `mlb_baseball/connectors/bref.py` pulls a regular-season-only Baseball-Reference window (ADR-282 / ADR-283); an `mlb doctor` envelope check fails on any row over 163 games. Current source coverage begins in 2008; unresolved provider player IDs are excluded and reported by health checks. The Baseball-Reference "official" season line — parallel to, never merged with, the event-derived `gold.batting_season` / `gold.pitching_season` (ADR-281). |
| `gold.team_season` | One final-season line per `(team_id, season)` | Final-season aggregate; never a pregame feature | Explicit `mlb report` rebuild from Lahman, core games, Retrosheet event aggregates, and conformed WAR. **Regular season only** (`mlb doctor` envelope check). Historical team coverage differs from player coverage and known unresolvable teams remain documented gaps. |
...
| `gold.batting_game` | One batting box-score line per `(game_id, player_id, team_id)`, regular season only; `source` marks the builder (`retrosheet_event` ≤ 2025, `mlb_boxscore` ≥ 2026) | Actual game result, not a pregame feature — this is the *what happened* grain, the complement to `gold.game_feature`'s *what was knowable before* | Explicit `mlb report` rebuild (truncate-and-replace, idempotent). **1910–2025** from `raw.retrosheet_event` (`sql/batting_game_build.sql`), using the same `bat_event_fl` / `ab_fl` / `sf_fl` / `sh_fl` / `event_cd` handling as `sql/team_woba_retrosheet_update.sql` (ADR-034). **2026 onward** from MLB's official per-game box score `raw.mlb_boxscore_batting` (`sql/batting_game_mlb_build.sql`, joined to `core.game` on `game_pk`) — Retrosheet publishes no event file for the in-progress season (ADR-289). The two builders share one `TRUNCATE`; the `g.season <= 2025` / `g.season >= 2026` bounds guarantee they never write the same key (an `mlb doctor` guard confirms it). Counting stats only; rate stats belong in the season/career roll-ups. Degrades gracefully when either source is absent. `gidp` undercounts pre-1988 (sparse `battedball_cd`). Postseason is a separate builder. First relation of the grain-complete statistic backbone (Plan 03B). |
| `gold.pitching_game` | One pitching box-score line per `(game_id, player_id, team_id)`, regular season only; `source` marks the builder | Actual game result, not a pregame feature | Explicit `mlb report` rebuild (truncate-and-replace, idempotent). **1910–2025** from `raw.retrosheet_event` (`sql/pitching_game_build.sql`): every play charged to `re.resp_pit_id`; runs charged per responsible pitcher (`resp_pit_id` for the batter-runner, `re.run{1,2,3}_resp_pit_id` for inherited runners). **2026 onward** from `raw.mlb_boxscore_pitching` (`sql/pitching_game_mlb_build.sql`, ADR-289). Counting stats only. **`er`** is NULL for 1910–2025 (the event stream carries no earned-run data — `r` and season RA9 are the honest figures) and populated from 2026 (MLB's scorer-assigned `earned_runs`); a documented coverage cliff. Same graceful-degrade, no-double-write guard, and `battedball_cd` caveats as `gold.batting_game`. Part of the grain-complete statistic backbone (Plan 03B, ADR-278). |
| `gold.batting_season` | One `is_combined = false` stint row per `(player_id, season, team_id)`, plus one `is_combined = true` combined row per `(player_id, season)` (`team_id` NULL); one-team players' combined row equals their stint | Actual season result; never a pregame feature | Explicit `mlb report` rebuild (truncate-and-replace, idempotent) rolled up from `gold.batting_game`. Counting stats are plain sums; rate stats (AVG/OBP/SLG/OPS/ISO/BABIP/BB%/K%) are computed from this grain's summed components and are NULL on a zero denominator. **SB/CS/SB% absent** — steals are not in `gold.batting_game` (deferred to `gold.baserunning_game`), even though the 2026 box score carries them. Regular season, 1910 onward (source-agnostic: 1910–2025 event-derived, 2026+ box-score-derived). Relation 3 of the grain-complete statistic backbone (Plan 03B, ADR-278). |
| `gold.batting_team` | One row per `(team_id, season)` | Actual season result; never a pregame feature | Explicit `mlb report` rebuild rolled up from `gold.batting_game`. Same counting + rate columns and caveats as `gold.batting_season`. Regular season, 1910 onward. Plan 03B. |
| `gold.pitching_season` | Same two-row-kind shape as `gold.batting_season` (stint rows + one `is_combined` combined row per `(player, season)`) | Actual season result; never a pregame feature | Explicit `mlb report` rebuild rolled up from `gold.pitching_game`. Counting stats plain sums; rate stats (RA9/WHIP/K9/BB9/HR9/K:BB) computed from this grain's summed components, NULL on a zero denominator. **`era`** is NULL through 2025 (no earned runs in the event stream — `ra9` is the cross-era rate) and populated from 2026, computed as `er * 27 / outs` only when every contributing game row carries `er`; a documented coverage cliff. Regular season, 1910 onward. Relation 4 of the grain-complete statistic backbone (Plan 03B, ADR-278). |
| `gold.pitching_team` | One row per `(team_id, season)` | Actual season result; never a pregame feature | Explicit `mlb report` rebuild rolled up from `gold.pitching_game`. Same counting + rate columns and caveats as `gold.pitching_season`, **except `er` / `era` are not carried at the team grain** (`ra9` is the team run rate for every year). Regular season, 1910 onward. Plan 03B. |
| `gold.batting_career` / `gold.pitching_career` | One row per `(player_id)` | Actual career result; never a pregame feature | Explicit `mlb report` rebuild summing each player's per-season combined rows (`is_combined`) from `gold.batting_season` / `gold.pitching_season`. Adds `seasons` / `first_season` / `last_season`; rate stats recomputed from career totals. `gold.pitching_career` carries `ra9` for every career; **`era` only for a wholly-2026+ career** (NULL unless every season has `er`) — same coverage cliff as `gold.pitching_season`. Regular season, 1910 onward. Relation 5 of the grain-complete statistic backbone (Plan 03B, ADR-278). |
| `gold.batting_postseason` / `gold.pitching_postseason` | One `is_combined = false`, `is_career = false` per-round row per `(player_id, season, round, team_id)`; one `is_combined = true` all-rounds row per `(player_id, season)`; one `is_career = true` row per `(player_id)` | Actual postseason result; never a pregame feature, never a regular-season number | Explicit `mlb report` rebuild (truncate-and-replace, idempotent) from **Lahman `BattingPost` / `PitchingPost`** (`raw.lahman_batting_post` / `raw.lahman_pitching_post`, 1884+). `playerid` → `core.player.bbref_id` with a `raw.lahman_people.retroid` fallback (~99.9% resolve, remainder counted by a health check); `teamid` + `yearid` → `core.team` via `raw.lahman_teams`. `round` kept verbatim. Same counting/rate columns as `gold.batting_season` / `gold.pitching_season` plus `sb`/`cs` (batting) and a **real `era`** (pitching — Lahman carries earned runs). Never contains a regular-season game; `mlb doctor` checks every `round` is a recognised postseason round. No postseason team-total relation yet (deferred `GROUP BY`). ADR-282 / ADR-283. |
| `gold.fangraphs_guts` | One row per `season` (`season` PK) | Reference / cross-check only; **never a pregame feature, never a published figure, never a reference-baseline-model input** | Explicit `mlb report` rebuild (`report._build_backbone_relation`, truncate-and-replace, idempotent) conformed **verbatim** from `raw.fangraphs_guts` — FanGraphs' Guts! per-season wOBA / FIP linear-weight constants (`woba`, `wobascale`, `wbb`, `whbp`, `w1b`, `w2b`, `w3b`, `whr`, `runsb`, `runcs`, `r_pa`, `r_w`, `cfip`). Text → numeric cast only; no re-derivation, no interpolation of missing seasons; empty string → NULL. Skips cleanly on a database that never ingested FanGraphs. **`local_research` — never `public_safe`, never in the published `mlb-research` dataset, never a baseline-model input.** A publishable wOBA / FIP is computed from `core.play`, not from here; the join is for era-accurate *internal* work only. `mlb doctor` checks every `gold.batting_season` season ≥ 2003 has a row. fangraphs-conform Beat 1 / ADR-290. |
| `gold.fangraphs_park_factors` | One row per `(season, team_id)` (composite PK), **`season >= 2003`** | Reference / cross-check only; **never a pregame feature, never a published figure, never a reference-baseline-model input** | Explicit `mlb report` rebuild (truncate-and-replace, idempotent) from `raw.fangraphs_park_factors` — FanGraphs' basic (`basic_5yr`, `pf_3yr`, `pf_1yr`) and component (`pf_1b`…`pf_iffb`, `pf_fip`) park factors, kept as published (numeric cast only). `team` is a FanGraphs nickname resolved to `core.team` via the `'fangraphs'` source block in `core.team_alias` (34 aliases; CLE / TBA / WAS carry multiple historically-accurate nicknames). A nickname with no alias produces **no row** and is surfaced by an `mlb doctor` join-coverage check — never a null or guessed team. Pre-2003 rows stay in `raw` (low value, unstable franchise set). Skips cleanly without the raw source. **`local_research`** — same posture as `gold.fangraphs_guts`. fangraphs-conform Beat 1 / ADR-290. |
