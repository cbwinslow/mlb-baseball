...
| `gold.player_season` | One provider final-season line per `(player_id, season, is_pitcher)` | Final-season aggregate; never a pregame feature | Explicit `mlb report` rebuild from Baseball-Reference season rows plus conformed WAR. Current source coverage begins in 2008; unresolved provider player IDs are excluded and reported by health checks. The Baseball-Reference "official" season line — parallel to, never merged with, the event-derived `gold.batting_season` / `gold.pitching_season` (ADR-281). |
