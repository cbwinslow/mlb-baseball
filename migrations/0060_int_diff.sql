...
-- docs/FEATURE_ADMISSION_QUEUE.md). Pure derived columns over already-
-- approved paired team features -- no new raw dependency.
--
-- Renumbered from 0059 to 0060 during rebase onto main after BSR-01
-- (migration 0059_team_bsr.sql) merged first -- the exact migration-
-- number collision documented in this PR's own description and ADR-081.

ALTER TABLE gold.game_feature ADD COLUMN win_pct_diff numeric;
ALTER TABLE gold.game_feature ADD COLUMN win_pct_10_diff numeric;
ALTER TABLE gold.game_feature ADD COLUMN pyth_wpct_diff numeric;
ALTER TABLE gold.game_feature ADD COLUMN elo_diff numeric;
ALTER TABLE gold.game_feature ADD COLUMN woba_diff numeric;
ALTER TABLE gold.game_feature ADD COLUMN wrc_plus_diff numeric;
...
-- rename or reposition an existing view column, so a new column must
-- always be appended last (see ADR-081's own BSR-01 note on this exact
-- Postgres constraint).
