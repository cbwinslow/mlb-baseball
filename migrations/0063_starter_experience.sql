...
-- PLN-04's "prior MLB PA/IP" half, docs/FEATURE_ADMISSION_QUEUE.md).
--
-- Renumbered from 0059 to 0063 (ADR from 081 to 085) during rebase onto
-- main: BSR-01 (0059/ADR-081), INT-01 (0060/ADR-082), and INT-02
-- (0061/ADR-083) are all real and merged as of this rebase. age.py
-- (0062/ADR-084) has not merged yet, so the view below extends
-- 0061_trend.sql's real current tail (...home_win_pct_trend,
-- away_win_pct_trend) directly -- it does NOT anticipate age.py's own
-- columns, since referencing a column that doesn't exist yet would fail
-- migrate.run() outright. Whichever of age.py/this branch merges second
-- will need to re-extend the view again from the other's real merged
-- state, the same pattern used for INT-02's own migration already this
-- session.

ALTER TABLE gold.game_feature ADD COLUMN home_starter_career_bf integer;
ALTER TABLE gold.game_feature ADD COLUMN away_starter_career_bf integer;
ALTER TABLE gold.game_feature ADD COLUMN home_starter_career_ip numeric;
ALTER TABLE gold.game_feature ADD COLUMN away_starter_career_ip numeric;
