...
-- (event_rows), and a window aggregate (not a second GROUP BY + JOIN)
-- resolves each team's starter over the same scan that also produces
-- per-pitcher outs -- confirmed row-for-row identical against production
-- to the old two-scan version (406,516/406,516 rows, zero mismatches
-- either direction) before landing this change.
...
event_rows AS (
    SELECT rg.game_id,
        ...
        max(re.resp_pit_id) FILTER (WHERE re.resp_pit_start_fl = 'T')
            OVER (PARTITION BY rg.game_id, CASE WHEN re.bat_home_id = '0'
