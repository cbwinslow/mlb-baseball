-- Coverage check (issue #32): team_rate_health_check.sql's range checks can
-- only ever catch a value that's present but out of range -- if a join
-- breaks entirely (e.g. a mismatched team_id), the affected columns go
-- NULL for every row, not out-of-range, and IS NOT NULL excludes NULLs from
-- the count, so the range checks report 0 bad rows even though the feature
-- is completely missing. home_pa/away_pa are the right target: they're
-- ungated (always populated whenever the join succeeds, unlike obp/slg/etc
-- which can be legitimately NULL below MIN_PA/MIN_AB/MIN_BIP) and every
-- other rate stat here rides on the same join, so a broken home_pa/away_pa
-- implies the whole side is broken. "Eligible" is reconstructed the same
-- way team_rate_retrosheet_update.sql itself defines it -- a team/game is
...
-- must gate on that themselves (same two-table gate as
-- team_rate.py::compute), since to_regclass('nonexistent') is NULL, not an
-- error, but joining a genuinely nonexistent relation still is.
...
SELECT
    count(*) FILTER (WHERE eh.eligible AND f.home_pa IS NULL),
    count(*) FILTER (WHERE ea.eligible AND f.away_pa IS NULL)
