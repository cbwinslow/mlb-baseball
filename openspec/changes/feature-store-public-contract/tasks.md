...
- [ ] 1.1 Update `build_features()`'s docstring (currently *"Rebuild
...
- [ ] 1.2 Check `mlb_baseball/__init__.py`'s re-export of `build_features` for
...
- [ ] 2.1 Change the `build_features()` table row (currently *"Rebuild
...
- [ ] 2.2 Add one sentence directly below the API table pointing readers to
...
- [ ] 3.1 Add one sentence to the `gold` bullet (currently ends "...`gold.game_feature`
...
- [ ] 4.1 `openspec validate feature-store-public-contract --strict` passes
  for the new `feature-store-boundary` capability delta.

## 5. Verification

- [ ] 5.1 Fresh read-through: `docs/PUBLIC_API.md`, `docs/ARCHITECTURE.md`,
...
- [ ] 5.2 `uv run pytest` targeted at any existing test asserting
