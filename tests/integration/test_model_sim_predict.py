...
def test_predict_skips_a_game_whose_simulation_fails_and_keeps_the_rest(db_conn, monkeypatch):
    # A genuinely degenerate estimated distribution raises
    # DegenerateSimulation out of simulate_matchup. That one game must be
    # skipped -- not abort the whole slate, and not roll back the shared
    # log5/Elo/GBM transaction.
    _ensure_retrosheet(db_conn)
    teams = _seed_teams(db_conn)
    gid = "NYA202304010"
    with db_conn.cursor() as cur:
    ...
    def flaky(conn, *, mlb_game_pk, **kwargs):
        if mlb_game_pk == "bad":
            raise markov.DegenerateSimulation("50/50 simulated games never broke a tie")
...
def test_predict_does_not_swallow_a_data_contract_markov_error(db_conn, monkeypatch):
    # A plain MarkovError (e.g. "no observed outcomes for state") is a data
    # or code defect affecting every matchup, not one bad game -- it must
    # abort the run loudly, not be counted as a skipped game.
    _ensure_retrosheet(db_conn)
    teams = _seed_teams(db_conn)
    gid = "NYA202304010"
    with db_conn.cursor() as cur:
        ...
        cur.execute(
            ...
            "VALUES ('g1', 'mlb:g1', 2024, '2024-07-01', %s, %s, NULL)",
            (teams["ATL"], teams["NYA"]),
        )
    db_conn.commit()

    def boom(*_args, **_kwargs):
        raise markov.MarkovError("no observed outcomes for state BaseOutState(...)")

    monkeypatch.setattr(sim_predict, "simulate_matchup", boom)
    with pytest.raises(markov.MarkovError, match="no observed outcomes"):
        sim_predict.predict(db_conn, n_games=20)
    db_conn.rollback()
